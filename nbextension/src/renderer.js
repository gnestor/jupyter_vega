...
export function register_renderer($) {
    ...
    return function(json, md, element) {
      ...
      const props = {
        ...
        renderedCallback: (error, result) => {
          ...
          if (this.outputs[0]) this.outputs[0].data['image/png'] = imageData;
        }
      };
  ...
  const index = 0;
  // Register the mime type and append_mime_type function with the notebook's OutputArea
  [VEGA_MIME_TYPE, VEGALITE_MIME_TYPE].forEach(mimetype => {
    OutputArea.register_mime_type(mimetype, append_mime(mimetype), {
      // Is output safe?
      safe: true,
...
export function render_cells($) {
  ...
  $('#notebook-container').find('.cell').toArray().forEach(item => {
    ...
    // If a cell has output data of 'application/vnd.vega.v2+json' mime type
    if (
